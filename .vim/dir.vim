...
"
" Vim file browser, slightly modified from sources by Ken Shan
" originally by Raul Segura Acevedo <raul@hplara.iquimica.unam.mx>
"
" User commands:
" `ls	loads vim's current directory.
" `lf	loads the current file's directory. (it doesn't work on a [No File]).
"
...
" the command.
"
"   `cp		Copy the file(s) to another place.
"   `mv		Move the file(s) to another place.
"   `ln		Link the file(s) to another place.
"   `sln	Link (symbolic) the file(s) to another place.
"   `mk		Make directories (the given files are used as a template)
"   `rm		Remove the file(s) (directories have to be empty)
"   `rf		Remove (force) the file(s) (rm -rf is used, directories are
...
"	 easily edited.
"
" `mk	 call __LSC to create a new window which prompts the user to create
"	 directories (the given files are used as a template) 
"
" `rf	 call __LSC to create a new window which prompts the user to delete
"	 the given files, and directories.  rm -rf is used!! directories are
"	 removed recursively.
" `rm	 same as RF but it uses rm on files and rmdir on directories, (so
...
au BufEnter */,.,.. nm `cp :cno__L_C- cp<CR>__LSC-1G0icp<Esc>__CJ2-
au BufEnter */,.,.. nm `mv :cno__L_C- mv<CR>__LSC-1G0imv<Esc>__CJ2-
au BufEnter */,.,.. nm `ln :cno__L_C- ln<CR>__LSC-1G0iln<Esc>__CJ2-
au BufEnter */,.,.. nm `sln :cno__L_C- ln-s<CR>__LSC-1G0iln -s<Esc>__CJ2-
au BufEnter */,.,.. nm `rf :cno__L_C- rm-rf<CR>__LSC-{d{{irm -rf<Esc>__CJ-
au BufEnter */,.,.. nm `rm :cno__L_C- rm<CR>__LSC-{kddmy:1,-2g-/$-m'y-<CR>:.g-/$-norm {j0irmdir<C-V><ESC>__CJ-<CR>:1g/./norm 0irm<C-V><Esc>__CJ-<CR>
au BufEnter */,.,.. nm `mk :cno__L_C- mk<CR>__LSC-{d{{imkdir__CJ-
...
au BufLeave */,.,.. unm %|unm`cp|unm`mv|unm`ln|unm`sln|unm`rf|unm`rm|unm__LSC-|unm`mk

map `ls :sp.<CR>3G$B
map `lf :sp%:p:h/<CR>3G$B
